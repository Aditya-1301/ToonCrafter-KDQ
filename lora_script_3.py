...
def compute_metrics(gt, pred):
    ...
    for g, p in zip(gt_np, pd_np):
        # FIX: Call peak_signal_noise_ratio only once to avoid redundant computation.
        psnr_val = peak_signal_noise_ratio(g, p, data_range=1.0)
        psnrs.append(psnr_val if np.isfinite(psnr_val) else 0.0)
        win_size = min(7, g.shape[0], g.shape[1])
        if win_size % 2 == 0: win_size -= 1
        ssim_val = ssim(g, p, data_range=1.0, channel_axis=2, win_size=win_size) if win_size >= 3 else 0.0
