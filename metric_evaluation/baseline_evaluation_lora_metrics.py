...
def load_model_checkpoint(model, ckpt, lora_ckpt_dir=None):
    ...
    try:
        ...
        print(f"Caught a RuntimeError loading base model, attempting with strict=False: {e}")
...
def get_latent_z(model, videos): #... same as before
...
def get_latent_z_with_hidden_states(model, videos): #... same as before
    b, c, t, h, w = videos.shape
    x = rearrange(videos, 'b c t h w -> (b t) c h w')
...
def image_guided_synthesis(model, prompts, videos, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
                        unconditional_guidance_scale=1.0, cfg_img=None, fs=None, text_input=False, multiple_cond_cfg=False, loop=False, interp=False, timestep_spacing='uniform', guidance_rescale=0.0, **kwargs): #... same as before
...
#     sample_save_dir = os.path.join(args.output_dir, "evaluation_samples_lora")
...
#         for i, batch in tqdm(enumerate(dataloader), desc="Evaluating LoRA Model"):
...
#             batch_samples = image_guided_synthesis(model, prompts, videos, noise_shape, 1, args.ddim_steps, args.ddim_eta, args.unconditional_guidance_scale, args.cfg_img, args.frame_stride, args.text_input, args.multiple_cond_cfg, args.loop, args.interp, args.timestep_spacing, args.guidance_rescale)
...
#     print("\n" + "="*40 + "\n      LORA MODEL EVALUATION COMPLETE\n" + "="*40)
...
# In evaluate_lora_model.py, replace the run_evaluation function with this one.
...
def run_evaluation(args):
    ...
    # We will ALWAYS use debug mode for this diagnostic run
    ...
    sample_save_dir = os.path.join(args.output_dir, "lora_diagnostic_samples")
    os.makedirs(sample_save_dir, exist_ok=True)
    print(f"DIAGNOSTIC VIDEOS will be saved in: {sample_save_dir}")

    all_psnr, all_ssim, all_lpips = [], [], []
    noise_shape = [1, model.model.diffusion_model.out_channels, args.video_length, args.height // 8, args.width // 8]
    
    with torch.no_grad(), torch.cuda.amp.autocast():
        for i, batch in tqdm(enumerate(dataloader), desc="Diagnosing LoRA Model"):
            start_frame = batch['start_frame'].unsqueeze(2).repeat(1, 1, args.video_length//2, 1, 1)
            end_frame = batch['end_frame'].unsqueeze(2).repeat(1, 1, args.video_length//2, 1, 1)
            videos = torch.cat([start_frame, end_frame], dim=2).cuda()
            prompts = [batch['prompt'][0]]
            ...
            # Save the generated video for inspection
            filename = f"diagnostic_sample_idx_{i}.mp4"
            save_results_seperate(prompts, batch_samples[0], filename, sample_save_dir, fps=8)
            print(f"\nSaved diagnostic video: {filename}")
            # --- END OF NEW PART ---
    ...
    avg_ssim = np.mean(all_ssim) if all_ssim else 0
    avg_lpips = np.mean(all_lpips) if all_lpips else 0
    print("\n" + "="*40 + "\n      LORA MODEL DIAGNOSIS\n" + "="*40)
    print(f"  PSNR: {avg_psnr:.2f}, SSIM: {avg_ssim:.4f}, LPIPS: {avg_lpips:.4f}")
    print("Check the 'lora_diagnostic_samples' directory to see the generated videos.")
    print("="*40 + "\n")

def get_parser():
    parser = argparse.ArgumentParser()
    parser.add_argument("--lora_ckpt_dir", type=str, required=True, help="Directory containing the trained LoRA adapter.")
